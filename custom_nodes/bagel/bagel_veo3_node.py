...
try:
    ...
    logger.debug(f"[BagelVeo3Node] Multi-user mode enabled")
except ImportError:
    # Fallback for self-hosted installations
    logger.debug(f"[BagelVeo3Node] Self-hosted mode (no middleware)")
...
logger.debug(f"[BagelVeo3Node] Backend URL: {BAGEL_BACKEND_URL}")
